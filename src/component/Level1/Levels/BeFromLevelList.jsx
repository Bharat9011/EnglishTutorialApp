...
const BeFromLevelList = () => {

  const Data = useSelector(state => state.subTopicList.BeForm);

  const lists = Data.map(data =>
    <div className='w-25 h-50 border border-3 border-primary my-4' key={data.id}>
      <Link to={"/MainLevelList/BeFromLevelList" + data.path} className='text-decoration-none'>
...
export default BeFromLevelList
